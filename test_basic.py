"""
Test script for Basic Fed-ProFiLA-AD
测试训练好的基础版本模型在测试集上的表现
加载检查点 -> 评估 -> 保存CSV -> 画图
"""
...
# ============================================================================
# 选择训练结果的方式（优先级从高到低）：
# 1. SELECT_INDEX: 通过序号选择（例如：1 表示最新的基础版本训练结果）
# 2. SELECT_TIMESTAMP: 通过时间戳选择（例如："20251031_153051"）
# 3. CHECKPOINT_PATH: 直接指定检查点路径（例如："outputs/run_20251031_153051/final_model.pt"）
# 注意：如果都设置为None，会自动选择最新的基础版本训练结果

SELECT_INDEX = 1  # 通过序号选择，例如：1 表示最新的基础版本
SELECT_TIMESTAMP = None  # 通过时间戳选择，例如："20251031_153051"
CHECKPOINT_PATH = None  # 直接指定完整路径，例如："outputs/run_20251031_153051/final_model.pt"
...
def test_trained_model(checkpoint_path: str, config_path: str = None, device_str: str = None):
    ...
    Args:
        checkpoint_path: 检查点路径
        config_path: 配置文件路径（可选，如果不提供则从检查点读取）
        device_str: 设备字符串（可选）
    """
    logger.info("=" * 60)
    logger.info("Testing Trained Basic Fed-ProFiLA-AD Model")
    logger.info("=" * 60)
    ...
    for client_id in client_ids:
        ...
        # 创建测试数据加载器（测试时使用单进程，避免多进程问题）
        test_loader = create_dataloader(
            root_path=root_path,
            client_id=client_id,
            is_train=False,
            batch_size=training_config['batch_size'],
            num_workers=0,  # 测试时强制使用单进程，避免Windows多进程问题
            pin_memory=False,
    ...
    plot_test_results(df, avg_metrics, plot_path)
...
def plot_test_results(df: pd.DataFrame, avg_metrics: Dict, save_path: Path):
    """
    绘制测试结果可视化图表
...
def find_available_checkpoints(outputs_dir: str = "outputs", run_type_filter: str = "basic"):
    """
    自动查找可用的检查点文件
    ...
    Returns:
        list: 检查点信息列表，每个元素包含 (run_name, checkpoint_path, run_type, timestamp)
    """
    ...
    for run_dir in outputs_path.iterdir():
        ...
        # 查找检查点文件（优先使用final_model.pt，如果没有则使用best_model.pt）
...
def list_available_checkpoints():
    """列出所有可用的检查点（只显示基础版本）"""
    checkpoints = find_available_checkpoints(run_type_filter="basic")
    
    if not checkpoints:
        print("\n未找到可用的基础版本检查点文件！")
        print("请确保 outputs/ 目录下有基础版本的训练结果（run_xxx格式）。")
        return
    ...
    print(f"  或者: python test_basic.py --select <序号>")
    print(f"  或者: python test_basic.py --checkpoint <完整路径>")
    print()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Test Basic Fed-ProFiLA-AD Model',
...
示例:
  ...
  # 列出所有可用的基础版本训练结果
  python test_basic.py --list
  
  # 选择序号为1的训练结果进行测试（命令行覆盖文件配置）
  python test_basic.py --select 1
  
  # 直接指定检查点路径（命令行覆盖文件配置）
  python test_basic.py --checkpoint outputs/run_20251101_094319/final_model.pt
        """
    )
    ...
    parser.add_argument('--list', action='store_true',
                       help='List all available basic version checkpoints')
    ...
    if args.checkpoint:
        ...
        if not matching:
            logger.error(f"未找到时间戳为 {SELECT_TIMESTAMP} 的基础版本训练结果！")
            list_available_checkpoints()
            return
        ...
        if not checkpoints:
            logger.error("未找到可用的基础版本检查点文件！")
            logger.error("请确保 outputs/ 目录下有基础版本的训练结果（run_xxx格式），")
        ...
        checkpoint_path = selected['checkpoint_path']
        logger.info(f"自动选择最新的基础版本: {selected['run_name']}")
        logger.info(f"检查点路径: {checkpoint_path}")
